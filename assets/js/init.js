...
function initCounters() {
    ...
    counters.forEach(counter => {
        ...
        // Определяем единицы измерения (%, Млн км², Млрд человек и т.д.)
        const unitMatch = rawText.match(/[^\d.,\s]+/);
        const unit = unitMatch ? unitMatch[0].trim() : '';
        
        // Определяем количество знаков после запятой
        const decimalPlaces = numberMatch[0].includes('.') ? numberMatch[0].split('.')[1].length : 0;
        ...
        const timer = setInterval(() => {
            ...
            // Форматируем число с сохранением единиц измерения
            let formattedNumber;
            if (Number.isInteger(target) && decimalPlaces === 0) {
                formattedNumber = Math.floor(current).toLocaleString('ru-RU');
            } else {
                formattedNumber = current.toFixed(decimalPlaces || 1);
            }
            
            counter.textContent = formattedNumber + (unit ? ' ' + unit : '');
        }, 16);
    });
}
